...
to setup
  ...
  ;;write headers to the file that records the network data
  write_headers_for_network_data
end
...
to go
  ...
  ;;write data about the network connections
  write_network_data
  tick
end
...
to write_headers_for_network_data
  file-open "/home/cbdavis/network.txt"
  file-print "tick\tFromNode\t ToNode"
  file-close
end

;;See http://groups.yahoo.com/group/netlogo-users/message/8074 for more info
to write_network_data
  file-open "/home/cbdavis/network.txt"
  ;;code for headers should only be done once
  ;;file-print "tick FromNode ToNode"
  let network map [ [ (list who [who] of link-neighbors ) ] of ? ] sort turtles
  foreach network
  [ 
    let fromnode first ?
    foreach but-first ?
    [
      foreach ?
      [
        ;;There's probably a smarter way to do this, but I can't figure out the syntax
        file-type ticks
        file-type "\t"
        file-type fromnode
        file-type "\t"
        file-type ?
        file-print "" ;;make a newline
      ]
    ]
  ]
  file-close
